...
// Bubbletea restores alt screen / bracketed paste / focus reporting after a
// tea.ExecProcess, but never the mouse-enable sequence. Coming back from the
// Plannotator TUI must not leave the terminal in mouse-off: drag-select and
// right-click would die silently until the user ran /mouse again.
func TestRestoreMouseCmdAfterTUIHandoff(t *testing.T) {
	m := New(nil, t.TempDir())
	m.ready = true

	m.Mouse = false
	if cmd := m.restoreMouseCmd(); cmd != nil {
		t.Error("mouse off must not re-enable mouse reporting")
	}

	m.Mouse = true
	if cmd := m.restoreMouseCmd(); cmd == nil {
		t.Error("mouse on must be re-asserted after a sub-TUI handoff")
	}
	if msg := m.restoreMouseCmd()(); msg != tea.EnableMouseCellMotion() {
		t.Errorf("restore cmd = %v, want tea.EnableMouseCellMotion", msg)
	}

	// Before the first render there is no terminal state to restore.
	m.ready = false
	if cmd := m.restoreMouseCmd(); cmd != nil {
		t.Error("not-ready model must not emit a mouse command")
	}
}
