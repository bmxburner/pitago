...
const (
	...
	// pi's own builtins never arrive via get_commands, so gotui intercepts
	// them locally instead of leaking "/..." text into the chat.
	OriginPi = "pi"
	// OriginGotui is gotui's own addition on top of pi.
	OriginGotui = "gotui"
)
...
// gotui's own /recent. Anything else (extension/prompt/skill commands,
...
func All() []app.Builtin {
	...
	all := []app.Builtin{
		...
		{
			Name: "recent", Desc: "Switch recent model (gotui)", Usage: "/recent",
			Origin: OriginGotui,
